...
デフォルトでは「schema にないが Pleasanter 側に存在する column」(orphan-column) は **保持**される（既存データ非破壊）。`--allow-destroy` を付けると更新時に該当 column を Pleasanter から削除する。

```bash
pleasync apply                  # デフォルト: orphan を残す
pleasync apply --allow-destroy  # orphan column を削除（DESTRUCTIVE）
```

site 自体の削除はサポート外（schema から model を消しても site は残る）。

## 後続フェーズ

完了済:
- Phase 2.5: where operator (equals/in) + orderBy
- Phase 2.6a: relation `include`
- Phase 2.6b: where operator 拡充 (not/notIn/contains/startsWith/endsWith/gt/gte/lt/lte)
- Phase 3.1: `pleasync plan`
- Phase 3.2: `pleasync apply`
- Phase 3.3: `--allow-destroy`
- Phase 4: `pleasync introspect` (siteId / SitePackage JSON)
